...
resource "azurerm_network_interface" "new_terraform_lb01_nic01" {
    name                      = "${var.environment_prefix}-efi-lb01_nic01"
...
resource "azurerm_virtual_machine" "new_terraform_lb01" {
    name                  = "${var.environment_prefix}-efi-lb01"
    location              = "eastus2"
    resource_group_name   = data.azurerm_resource_group.existing_resource_group.name
    ...
    storage_os_disk {
        name              = "${var.environment_prefix}-efi-lb01_osDisk"
        caching           = "ReadWrite"
    ...
    os_profile {
        computer_name  = "${var.environment_prefix}-efi-lb01"
