...
resource "azurerm_network_interface" "new_terraform_worker02_nic01" {
    name                      = "demo-aud-worker02_nic01"
...
#########################################################################################
#VM Creation
resource "azurerm_virtual_machine" "new_terraform_worker02" {
    name                  = "demo-aud-worker02"
    location              = "eastus2"
    resource_group_name   = data.azurerm_resource_group.existing_resource_group.name
    network_interface_ids = ["${azurerm_network_interface.new_terraform_worker02_nic01.id}"]
    vm_size               = "Standard_B2ms"

    storage_os_disk {
        name              = "demo-aud-worker02_osDisk"
        caching           = "ReadWrite"
    ...
    os_profile {
        computer_name  = "demo-aud-worker02"
        admin_username = "aud"
